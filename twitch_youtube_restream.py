...
USHER_API = 'http://usher.twitch.tv/api/channel/hls/{channel}.m3u8?player=twitchweb' +\
    ...
    '&type=any&p={random}'
TOKEN_API = 'https://api.twitch.tv/api/channels/{channel}/access_token?client_id={twitch_api_client_id}'.format(twitch_api_client_id=config['twitch_api_client_id'], channel=config['twitch_channel_name'])
...
#    args = parser.parse_args()
    ...
    while config['continuous_retry']:
        ...
        if len(m3u8_obj.playlists) > 0:
            ...
            os.system("cvlc {} --sout '#transcode{{vcodec=FLV1,acodec=mp3,samplerate=44100}}:std{{access=rtmp,mux=ffmpeg{{mux=flv}},dst=rtmp://stream3.vkuserlive.com:443/live?srv=501717&s=aWQ9aFlmR3RJZEhlSnMmc2lnbj0yODZSTEVxbE1RL2wyaSt5Tlc3M3FRPT0='{}".format(url, config['youtube_stream_key']))
        else:
